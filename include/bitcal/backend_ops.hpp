...
struct ops<Bits, simd_backend::scalar> {
    ...
    static BITCAL_FORCEINLINE bool is_zero(const uint64_t* data) noexcept { return scalar::is_zero_array<N>(data); }

    static BITCAL_FORCEINLINE bool all(const uint64_t* data) noexcept { return scalar::all_array<N>(data); }
