...
> **硬件变更说明**：上表"修复前"数据来自旧 retained 基线（Intel i7-12700，commit `007c74071b25`），"修复后"数据来自新 retained 基线（AMD Ryzen 7 5800H，commit `27e0742`）。两次测量跨硬件，提升倍数含硬件差异；纯软件收益应在同机对照下另行量化。
>
> **可追溯性说明**：`007c74071b25` 不在当前仓库历史中（`git cat-file -t` 返回非有效对象名），可能来自上游或被 rebase 重写。该 hash 仅作为旧 JSON 的元数据记录保留，无法 `git checkout` 重现"修复前"基准。`27e0742` 可正常追溯。
