...
│  (lives in include/bitcal/bitarray.hpp)                    │
├─────────────────────────────────────────────────────────────┤
...
├── bitarray.hpp      # retained bitarray contract (implementation header)
...
Notes:
- `bitarray.hpp` is the physical home of the retained bitarray contract, but is an implementation header not a separate stable public include seam.
- The stable include seam remains `bitcal.hpp`.
- Backend-specific headers such as `sse_ops.hpp`, `avx_ops.hpp`, `avx512_ops.hpp`, and `neon_ops.hpp` are implementation details, not public entries.
