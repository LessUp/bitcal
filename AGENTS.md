...
BitCal 是一个 C++23、header-only、x86-64-first SIMD 加速的位操作库。当前项目目标不是继续扩张功能面，而是把仓库收敛到**可归档、可交接、可稳定维护**的终局状态。
...
- `bit_block<Bits>` / `bit_view` / `const_bit_view` 与自由算法是核心抽象；所有公开契约变化都要同步更新 API 规范与用户文档
